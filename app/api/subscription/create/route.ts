...
export async function POST(request: NextRequest) {
  try {
    ...
    if (cookieHeader) {
      ...
      // Storefront uses vendor_id + vendor_email
      // Dashboard uses user_id + user_email
      userId = cookies.vendor_id || cookies.user_id || null;
      userEmail = cookies.vendor_email || cookies.user_email || null;
    }

    if (!userId) {
      console.error("[SUBSCRIPTION] User ID not found in cookies");
      return NextResponse.json(
        { status: "error", message: "User ID not found. Please try again." },
        { status: 401 }
      );
    }
    ...
    if (!userEmail) {
      ...
      return NextResponse.json(
        { status: "error", message: "User email not found. Please try again." },
        { status: 401 }
      );
    }
    ...
    try {
      ...
      if (fetchError.name === "AbortError") {
        ...
        return NextResponse.json(
          { status: "error", message: "Subscription request timed out. Please try again." },
          { status: 408 }
        );
      }
      ...
      return NextResponse.json(
        { status: "error", message: "Unable to connect to subscription service. Please try again later." },
        { status: 503 }
      );
    }
